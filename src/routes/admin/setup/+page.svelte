...
<svelte:head>
	<title>{data.appName} Admin · Create Account</title>
...
>
	...
	<p class="card-sub">Set up the first administrator account for this deployment.</p>
	...
	<button type="submit" class="submit-btn" class:is-loading={submitting} disabled={submitting}>
		<span class="btn-label">{submitting ? 'Creating…' : 'Create Account'}</span>
		<svg viewBox="0 0 24 24" fill="currentColor"><path d="M6 4.5v15l13-7.5-13-7.5Z"/></svg>
	</button>

	<p class="card-footnote">This can only be done once — subsequent visits go to sign in.</p>
</form>
</div>
